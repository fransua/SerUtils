...
from serutils.sequence.seq_stats import nucleotide_content


def seq_logo(seqs=None, seq_count=None, letters=('A', 'T', 'G', 'C', 'N'),
             nseq=None, title='', axe=None, quality=1, plot=True, ylim=(0, 2),
             alpha=1, lwmodif=1.25, remove_gap_only=True, min_count=1,
    ...
    :params None savefig: if set save the plot to the file at the
        specified path (the format of the file "pdf", "png"... will be infered
    ...
    def A(x, y, h):
        if yoffset < h:
            ...
            h -= yoffset
        lwh = lw(h)
        axe.plot([x, x + halfw, x + letterw ], [y, y + h, y], color='green',
                 alpha=alpha, lw=lwh)
    ...
    if savefig:
        ...
        plt.close('all')
    # return seq_count
    return seq_count, table_heights
